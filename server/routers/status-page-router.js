...
router.get("/api/status-page/heartbeat/:slug", cache("1 minutes"), async (request, response) => {
    ...
    try {
        ...
        for (let monitorID of monitorIDList) {
            ...
            let interval = monitor ? monitor.interval : 60;
            ...
            heartbeatList[monitorID] = list.reverse().map((row) => {
                let status = 1;
                if (row.down > 0) status = 0;
                else if (row.up === 0 && row.down === 0) status = 2;
                
                let downtimeString = "No downtime";
                if (row.down > 0) {
                    let downtimeMinutes = Math.max(1, Math.round(row.down * (interval / 60)));
                    let hours = Math.floor(downtimeMinutes / 60);
                    let minutes = Math.floor(downtimeMinutes % 60);
                    if (hours > 0 && minutes > 0) downtimeString = `${hours}h ${minutes}m downtime`;
                    else if (hours > 0) downtimeString = `${hours}h downtime`;
                    else downtimeString = `${minutes}m downtime`;
                } else if (row.up === 0 && row.down === 0) {
                    downtimeString = "No data";
                }
                
                return {
                    status: status,
                    time: dayjs.unix(row.timestamp).toISOString(),
                    ping: row.ping || 0,
                    msg: downtimeString
                };
            });
