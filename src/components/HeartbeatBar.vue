...
export default {
    ...
    methods: {
        ...
        resize() {
            if (this.$refs.wrap) {
                if (this.normalizedHeartbeatBarDays > 0) {
                    this.maxBeat = this.normalizedHeartbeatBarDays;
                    this.beatWidth = (this.$refs.wrap.clientWidth / this.maxBeat) - (this.beatHoverAreaPadding * 2);
                } else {
        ...
        drawCanvas() {
            ...
            const dpr = (window.devicePixelRatio || 1) * 3; // Multiplied by 3 for crisp rendering when zoomed
            ...
            this.shortBeatList.forEach((beat, index) => {
                ...
                // Flat rectangular bars for a continuous look
                ...
                if (this.hoveredBeatIndex !== -1 && beat !== 0) {
...
.hp-bar-big {
    --beat-empty-color: #e4e4e7;

    .dark & {
        --beat-empty-color: #3f3f46;
    }
